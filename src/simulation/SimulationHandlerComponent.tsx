...
export const SimulationHandlerComponent: React.FC<
  SimulationHandlerComponentProps
> = ({ controller }) => {
  const [handler] = React.useState(() => new SimulationHandler(controller));
  const [clientSimulations, setClientSimulations] = useState<
    MAIN.ClientSimulationController[]
  >([]);

  useEffect(() => {
    setClientSimulations([
      ...
      >
        ...
        >
          ...
          >
            <h3>Server View</h3>
            <SimulationComponent
              simulation={handler.getController()}
              handler={handler}
            />
          </div>
